...
def argparser():
    ...
    parser.add_argument(
        "--exp-name", dest="exp_name", help="Experiment Name", required=True, type=str
    )
...
def save_plot_values(
    ...
    for i in range(len(temp_arrays)):
        ...
        if saveInTextFormat:
            ...
            np.savetxt(
                temp_dir + "/" + temp_names[i] + ".txt", temp_arrays[i], fmt="%1.2f"
            )
        else:
...
def main(cfg):
    ...
    data_obj = Data(cfg)
    train_data, train_size = data_obj.getDataset(
        save_dir=cfg.DATASET.ROOT_DIR, isTrain=True, isDownload=True
    )
    test_data, test_size = data_obj.getDataset(
        save_dir=cfg.DATASET.ROOT_DIR, isTrain=False, isDownload=True
    )
    ...
    lSet_path, uSet_path, valSet_path = data_obj.makeLUVSets(
        train_split_ratio=cfg.ACTIVE_LEARNING.INIT_L_RATIO,
    ...
    test_loader = data_obj.getTestLoader(
        data=test_data, test_batch_size=cfg.TRAIN.BATCH_SIZE, seed_id=cfg.RNG_SEED
    )

    wandb.init(project="SSL-AL")
    wandb.config.update(cfg)

    # Initialize the model.
    model = model_builder.build_model(cfg)
    print("model: {}\n".format(cfg.MODEL.TYPE))
    if cfg.ACTIVE_LEARNING.MODEL_LOAD_DIR != "":
        model = cu.load_checkpoint(cfg.ACTIVE_LEARNING.MODEL_LOAD_DIR, model)
        print("Loaded model from: {}\n".format(cfg.ACTIVE_LEARNING.MODEL_LOAD_DIR))
    logger.info("model: {}\n".format(cfg.MODEL.TYPE))
...
def train_model(train_loader, val_loader, model, optimizer, cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        ...
        print(
            "Training Epoch: {}/{}\tTrain Loss: {}\tVal Accuracy: {}".format(
                cur_epoch + 1,
                cfg.OPTIM.MAX_EPOCH,
                round(train_loss, 4),
                round(val_set_acc, 4),
        ...
        wandb.log({"Val Set Accuracy": val_set_acc})

    # Save the best model checkpoint (Episode level)
    checkpoint_file = cu.save_checkpoint(
        info="vlBest_acc_" + str(int(temp_best_val_acc)),
...
def test_model(test_loader, checkpoint_file, cfg, cur_episode):
    ...
    test_acc = 100.0 - test_err

    plot_episode_xvalues.append(cur_episode)
    plot_episode_yvalues.append(test_acc)

    plot_arrays(
        x_vals=plot_episode_xvalues,
        y_vals=plot_episode_yvalues,
        x_name="Episodes",
    ...
    save_plot_values(
        [plot_episode_xvalues, plot_episode_yvalues],
        ["plot_episode_xvalues", "plot_episode_yvalues"],
        out_dir=cfg.EXP_DIR,
    )
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels) in enumerate(train_loader):
        ...
        if cur_iter != 0 and cur_iter % 19 == 0:
            ...
            print(
                "Training Epoch: {}/{}\tIter: {}/{}".format(
                    cur_epoch + 1, cfg.OPTIM.MAX_EPOCH, cur_iter, len(train_loader)
                )
            )
...
def test_epoch(test_loader, model, test_meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels) in enumerate(test_loader):
        ...
        test_accuracy = 100 - (misclassifications / totalSamples * 100)
        wandb.log({"Epoch": cur_epoch + 1, "Test Accuracy": test_accuracy})
